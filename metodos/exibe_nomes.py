from models import exibe_motorista,exibe_placa, exibe_viagem, tipo_pessoa


SQL_BUSCAR_MOTORISTA = '''select id_motorista, nome_motorista, email from registrar_motorista'''

SQL_BUSCAR_PLACA = 'select id_veiculo, placa_veiculo from registrar_carro'

SQL_TIPO_PESSOA = 'select * from tipo_pessoa'
...
class exibe:
    ...
    def exibe_tipo_pessoa(self):
        cursor = self.__db
        cursor.execute(SQL_TIPO_PESSOA)
        tipo_pessoa = traduz_tipopessoa(cursor.fetchall())
        return tipo_pessoa
...
def traduz_nome(lista_motorista):
    def cria_usuario_com_tupla(tupla):
        ...
        return exibe_motorista(tupla[0], tupla[1], tupla[2])
...
def traduz_tipopessoa(lista_tipopessoa):
    def cria_usuario_com_tupla(tupla):
        
        
        return tipo_pessoa(tupla[0], tupla[1])
        
    return list(map(cria_usuario_com_tupla, lista_tipopessoa))